...
GO.wall %>% 
  ...
  geom_point() +
  expand_limits(x=0) +
  labs(x="Genen", y="Functie", colour="p value", size="Count")+ scale_colour_gradient(low = "hotpink", high = "yellow")
BiocManager::install("pathview")
library(pathview)
pathview(gene.data = gene.vector, pathway.id = "hsa04612", species = "hsa", gene.idtype = "SYMBOL", limit = list(gene = 5))
